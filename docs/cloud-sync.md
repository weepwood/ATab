...
手动会话和云收藏的新增、修改、导入和删除会与本地业务数据写入放在同一 Dexie 事务中。
...
- 接受服务端：将服务端快照写入对应本地业务表，并删除冲突 Outbox。
...
- 原生书签仍由浏览器及其账号同步机制管理；
- 用户可以在云收藏页预览并复制导入原生书签；
- 导入不会移动、修改或删除原书签；
- 云收藏使用 `cloud-bookmark` 实体参与 ATab 增量同步；
- 删除云收藏不会影响原生书签。
...
- 未完成真实 Supabase 环境的迁移与两设备集成测试；当前 CI 覆盖 TypeScript 协议、API 路由和客户端构建。
- 云收藏文件夹目前使用路径字段，尚未提供独立文件夹实体和层级拖拽。
