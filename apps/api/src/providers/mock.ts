...
function duplicatePlan(tabs: AgentTabContext[]): AgentPlanDraft {
  ...
  return {
    ...
    operations: tabIds.length === 0 ? [] : [{
      type: 'CLOSE_TABS', tabIds, name: null, color: null,
    }],
  }
}

function githubPlan(tabs: AgentTabContext[]): AgentPlanDraft {
  const tabIds = tabs.filter((tab) => new URL(tab.url).hostname.endsWith('github.com')).map((tab) => tab.id)
  return {
    ...
    requiresConfirmation: true,
    operations: tabIds.length === 0 ? [] : [{
      type: 'CREATE_GROUP', tabIds, name: '开发', color: 'blue',
    }],
  }
}
...
function mutePlan(tabs: AgentTabContext[]): AgentPlanDraft {
  ...
  return {
    ...
    requiresConfirmation: true,
    operations: tabIds.length === 0 ? [] : [{
      type: 'MUTE_TABS', tabIds, name: null, color: null,
    }],
  }
}
