  ...
  <section class="view-content">
    <header class="page-header">
      ...
      <div class="header-actions">
        ...
        <button class="ghost-button" :disabled="store.mutating" @click="fileInput?.click()">导入 JSON</button>
        <button class="ghost-button" :disabled="store.mutating" @click="openSave('current')">保存当前窗口</button>
        <button class="primary-button" :disabled="store.mutating" @click="openSave('all')">保存全部窗口</button>
      </div>
    </header>
    ...
    <section class="snapshot-note surface">
      <div>
        ...
        <p>每 30 分钟保存一次全部普通窗口，只保留最近 10 份；单个会话最多 20 个窗口和 500 个标签。</p>
      </div>
      <button class="ghost-button" :disabled="store.loading || store.mutating" @click="store.refresh">刷新列表</button>
    </section>
    ...
    <p v-if="store.lastRestore" class="result-banner">
      ...
      <template v-if="store.lastRestore.skippedGroups">，{{ store.lastRestore.skippedGroups }} 个标签组未能恢复</template>。
    </p>
        ...
        <div v-else class="session-grid">
          <article v-for="session in store.manualSessions" :key="session.id" class="session-card surface">
            ...
            <footer>
              ...
              <button class="ghost-button" :disabled="store.mutating" @click="exportSession(session)">导出</button>
              <button class="ghost-button" :disabled="store.mutating" @click="openRename(session)">重命名</button>
              <button class="danger-button" :disabled="store.mutating" @click="openDelete(session)">删除</button>
            </footer>
          </article>
        </div>
      </section>
    ...
    <div v-if="dialogMode" class="dialog-backdrop" @click.self="closeDialog">
      <section class="dialog surface" role="dialog" aria-modal="true">
        <template v-if="dialogMode === 'save'">
          ...
          <p>仅保存 HTTP、HTTPS 和本地文件页面；恢复本地文件需要浏览器允许扩展访问文件网址。</p>
          <label>会话名称<input v-model="sessionName" class="input" maxlength="200" placeholder="留空将按时间自动命名" /></label>
        </template>

        <template v-else-if="dialogMode === 'rename' && selectedSession">
          <h2>重命名会话</h2>
          <label>会话名称<input v-model="sessionName" class="input" maxlength="200" /></label>
        </template>
