  ...
  properties: {
    ...
    operations: {
      ...
      items: {
        ...
        properties: {
          type: { type: 'string', enum: ['CREATE_GROUP', 'CLOSE_TABS', 'MUTE_TABS'] },
          tabIds: {
            type: 'array',
            maxItems: 500,
          ...
          color: { type: ['string', 'null'], enum: [...TAB_GROUP_COLORS, null] },
        },
      },
    },
  },
...
export function validateAgentPlanRequest(value: unknown): AgentPlanRequest {
  ...
  const tabs = input.tabs.map((tab, index) => {
    ...
    seenIds.add(id)
    const url = readString(item.url, `tabs[${index}].url`, 8_000)
    const parsedUrl = new URL(url)
    if (parsedUrl.protocol !== 'http:' && parsedUrl.protocol !== 'https:') {
      throw new Error(`tabs[${index}].url 仅支持 HTTP 或 HTTPS`)
    }
    return {
      id,
      windowId: readTabId(item.windowId, `tabs[${index}].windowId`),
  ...
  return {
    ...
    tabs,
    locale: input.locale === undefined ? undefined : readString(input.locale, 'locale', 32),
  }
}
...
export function normalizeAgentPlanDraft(
  ...
  const usage = new Map<number, Set<AgentOperation['type']>>()

  for (const [index, operation] of input.operations.entries()) {
    const item = asObject(operation, `operations[${index}]`)
    const type = readOperationType(item.type, `operations[${index}].type`)
    if (!Array.isArray(item.tabIds)) throw new Error(`operations[${index}].tabIds 必须是数组`)
    ...
    for (const id of tabIds) {
      const used = usage.get(id) ?? new Set<AgentOperation['type']>()
      if (used.has(type)) throw new Error(`标签 ${id} 被重复用于 ${type}`)
      if (type === 'CLOSE_TABS' && used.size > 0) {
        throw new Error(`标签 ${id} 不能同时关闭并执行其他操作`)
      }
      if (used.has('CLOSE_TABS')) {
        throw new Error(`标签 ${id} 不能同时关闭并执行其他操作`)
      }
      if (type === 'CREATE_GROUP' && used.has('CREATE_GROUP')) {
        throw new Error(`标签 ${id} 不能同时加入多个分组`)
      }
      used.add(type)
      usage.set(id, used)
    }
    ...
    if (type === 'CREATE_GROUP') {
      operations.push({
        type,
        tabIds,
        name: readString(item.name, `operations[${index}].name`, 80),
        color: readTabGroupColor(item.color, `operations[${index}].color`),
      })
    } else {
...
function deriveRisk(operations: AgentOperation[]): AgentRisk {
  ...
  return operations.length > 0 ? 'reversible' : 'read-only'
}

function readOperationType(value: unknown, label: string): AgentWireOperation['type'] {
  if (value === 'CREATE_GROUP' || value === 'CLOSE_TABS' || value === 'MUTE_TABS') return value
...
function readString(value: unknown, label: string, maxLength: number, allowEmpty = false): string {
