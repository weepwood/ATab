...
- 在一个入口搜索标签页、书签、云收藏、网页资料、会话和已授权历史。
- 通过统一 `resource` 模型对网页去重，避免标签页、书签和历史重复索引。
- 让 AI 生成可审计的结构化操作计划，由扩展验证并执行。
- 默认本地保存，正文采集、历史读取、AI 摘要和语义查询均需单独授权或主动操作。
...
- 用户逐条建立的资源语义索引：显示 endpoint 与发送字符数，向量仅保存到 IndexedDB。
- 默认关闭的语义查询：只有点击“语义查找”才发送当前查询词。
- 同 Provider、模型、维度和正文哈希约束下的本地余弦相似度计算。
- 关键词结果与语义结果本地融合、按资源 ID 去重，并在语义服务失败时保留关键词结果。
...
- 兼容严格 JSON Schema 和标准 Embeddings 接口的远程 Provider。
- 扩展与 API 共用操作协议、摘要协议、嵌入协议和运行时校验。
