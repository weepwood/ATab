const datasets = {
  tabs: [
    { id: 'tab-github', icon: 'G', title: 'weepwood / ATab · GitHub', detail: 'github.com/weepwood/ATab', tag: '开发', action: '固定' },
    { id: 'tab-vue', icon: 'V', title: 'Vue 3 Composition API', detail: 'vuejs.org/guide/extras/composition-api-faq', tag: '文档', action: '关闭' },
    { id: 'tab-fastify', icon: 'F', title: 'Fastify Documentation', detail: 'fastify.dev/docs/latest', tag: '开发', action: '静音' },
    { id: 'tab-supabase', icon: 'S', title: 'Supabase Auth & RLS', detail: 'supabase.com/docs/guides/auth', tag: '后端', action: '关闭' },
    { id: 'tab-research', icon: 'A', title: 'AI Browser Workspace Research', detail: 'example.invalid/research/ai-browser-workspace', tag: '研究', action: '固定' },
    { id: 'tab-pages', icon: 'D', title: 'GitHub Pages Deployment', detail: 'docs.github.com/pages', tag: '部署', action: '关闭' },
  ],
  sessions: [
    { id: 'session-release', icon: '◫', title: 'ATab 发布准备', detail: '8 个标签页 · 12 分钟前', tag: '自动快照', action: '恢复' },
    { id: 'session-research', icon: '◫', title: 'AI 浏览器研究', detail: '14 个标签页 · 昨天', tag: '手动保存', action: '恢复' },
    { id: 'session-sync', icon: '◫', title: 'Supabase 同步实现', detail: '6 个标签页 · 3 天前', tag: '手动保存', action: '恢复' },
  ],
  bookmarks: [
    { id: 'bookmark-atab', icon: '◇', title: 'ATab 项目仓库', detail: 'github.com/weepwood/ATab', tag: '项目', action: '打开' },
    { id: 'bookmark-mv3', icon: '◇', title: 'Chrome Extensions MV3', detail: 'developer.chrome.com/docs/extensions', tag: '文档', action: '打开' },
    { id: 'bookmark-dexie', icon: '◇', title: 'Dexie IndexedDB', detail: 'dexie.org/docs', tag: '本地优先', action: '打开' },
    { id: 'bookmark-provider', icon: '◇', title: 'OpenAI-compatible API', detail: 'example.invalid/provider-guide', tag: 'AI', action: '打开' },
  ],
  resources: [
    { id: 'resource-architecture', icon: '▤', title: '本地优先浏览器工作台架构', detail: '已采集 8,421 字 · 摘要已生成', tag: '架构', action: '查看' },
    { id: 'resource-security', icon: '▤', title: 'Manifest V3 安全边界', detail: '已采集 5,203 字 · 语义索引已建立', tag: '安全', action: '查看' },
    { id: 'resource-model', icon: '▤', title: '浏览器数据统一资源模型', detail: '已采集 6,880 字 · 摘要需更新', tag: '数据模型', action: '查看' },
    { id: 'resource-ai', icon: '▤', title: 'AI 结构化操作计划', detail: '已采集 4,126 字 · 摘要已生成', tag: 'AI', action: '查看' },
  ],
}
...
function renderListView(meta, view) {
  ...
  const actions = view === 'tabs'
    ? '<button class="pill-button active" data-filter="all">全部</button><button class="pill-button" data-filter="开发">开发</button><button class="pill-button" data-filter="研究">研究</button>'
    : ''

  elements.content.innerHTML = `
    ${sectionHead(meta, actions)}
    <div class="item-list">
      ${filtered.length ? filtered.map((item) => itemRow(item, view)).join('') : emptyState('没有匹配的数据', '调整搜索词或切换其他数据源。')}
    </div>
  `
  ...
  elements.content.querySelectorAll('.pill-button[data-filter]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      elements.content.querySelector('.item-list').innerHTML = items.map((item) => itemRow(item, view)).join('')
      bindRowActions(view)
    })
  })
  bindRowActions(view)
    ...
    <div class="item-list">
      ${results.length ? results.map((item) => `
        <article class="item-row">
          ...
          <div class="item-meta"><span class="tag">${escapeHtml(item.source)}</span><button class="row-action" data-generic-action="打开">→</button></div>
        </article>
  ...
  return `
    <article class="item-row" data-id="${escapeHtml(item.id)}">
      ...
      <div class="item-meta">
        ...
        <button class="row-action" data-row-action="${escapeHtml(item.action)}" data-id="${escapeHtml(item.id)}" data-view="${view}" aria-label="${escapeHtml(item.action)}">⋯</button>
      </div>
    </article>
  `
}
  ...
  elements.content.querySelectorAll('[data-row-action]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (view === 'tabs' && action === '关闭') {
        const index = state.tabs.findIndex((item) => item.id === button.dataset.id)
        if (index < 0) return
